...
{
    public class OneOfIgnoreNullAttribute : ValidationAttribute
    {
        private int[] ItemInt { get; set; }
        private string[] ItemString { get; set; }

        public OneOfIgnoreNullAttribute(params int[] items) : base("{0} value is not in " + String.Join(",", items))
        {
            this.ItemInt = items;
        }

        public OneOfIgnoreNullAttribute(params string[] items) : base("{0} value is not in " + String.Join(",", items))
        {
